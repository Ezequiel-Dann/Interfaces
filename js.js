...
document.addEventListener("keydown",(e)=>{
    ...
    if(moviendo!=null){
        let ahora = new Date();
        aceleracion = (ahora.getTime()-timeInicio.getTime())/300;
        console.log(aceleracion);
        if(e.key=="ArrowUp"){
            moviendo.setY(moviendo.getY()-1*aceleracion);      
        }
        if(e.key=="ArrowRight"){
            moviendo.setX(moviendo.getX()+1*aceleracion);
        }
        if(e.key=="ArrowDown"){
            moviendo.setY(moviendo.getY()+1*aceleracion);
        }
